...
let modelPhaseIndex=(d.modelphase>0 && d.modelphase-1<ph.count && ph.counts[d.modelphase-1]>0)
  ...
  while(soldCount<L){
    ...
    while(globalCredit>=1-1e-9&&soldCount<L){
      ...
      if(V){
        ...
        planned.push({product,phase:i,salesPhase:null,contractMonth:month,baselineBuildStart,buildStart:baselineBuildStart,baselineDelivery,delivery:baselineDelivery,saleBase,phaseEsc,salePrice,depositPct,buildCost:0,contCost:0,permitCost:0,baseBuild:0});
      }else{
        const lotSettlementLag=Math.max(0,Math.round(d.dellag)),baselineDelivery=month+lotSettlementLag;
        planned.push({product,phase:i,salesPhase:null,contractMonth:month,buildStart:0,baselineDelivery,delivery:baselineDelivery,saleBase,phaseEsc,salePrice,depositPct,buildCost:0,contCost:0,permitCost:0,baseBuild:0});
      }
  ...
  const K=["build","design","permit","vert1","land","closing","firb","dd","soft","supervision","professionalfee","dmfee","pidprocess","otherdirect","munic","impact","infra","cont","vcont","devc","fdev","sval","recog","dep","escr","escbal","balr","lsr","pid","sell","sellcash","brokeragecash","sellingexpensecash","outputgst","gstrefund","landgstrefund","devgstrefund","inputgstpaid","gstcogs","capex","ovh","fixovh","gna","gnashared","staff","staffshared","corpovh","marketing","da","otherincome","carry","retax","hoaexp","insurance","retaxcapadd","retaxcaprel","retaxexp","hoacapadd","hoacaprel","hoaexppl","insurancecapadd","insurancecaprel","insuranceexp","pf","pfamort","lf","lop","dr1","dr2","cumd","intr","icash","rep","lcl","net","taxm","eqcf","eqin","eqout","eqreimbcap","cash","price","cinf","model","landcogs","concogs","vertcogs","othcogs","landbasecogs","infracogs","designcogs","authoritycogs","constructioncogs","otherdirectcogs","finexp","fincapadd","fincaprel","relbase","relreq","pidcogs","contcogs","bradd","brcomplete"];
  ...
  const infTot=R.cinf.reduce((a,b)=>a+b,0),horizCont=R.cont.reduce((a,b)=>a+b,0),pid=R.pid.reduce((a,b)=>a+b,0);
  ...
  unitRecords.forEach(u=>{const p=products[u.product];if(V){const buildWeights=_spendWeights(p.buildMonths,d.buildcurve);for(let k=0;k<p.buildMonths;k++){const m=u.buildStart+k,c=u.baseBuild*buildWeights[k]*Math.pow(1+d.infesc/100,m/12);const vcontSpend=c*d.contpc/100;R.build[m]+=c;R.vcont[m]+=vcontSpend;u.buildCost+=c;u.contCost+=vcontSpend}u.permitCost=d.permpc/100*(d.permitbase?u.buildCost:u.baseBuild*Math.pow(1+d.infesc/100,u.contractMonth/12));R.permit[u.contractMonth]+=u.permitCost}});
  const actualBuild=R.build.reduce((a,b)=>a+b,0),designTot=0,vertCont=R.vcont.reduce((a,b)=>a+b,0),contTot=horizCont+vertCont;
  ...
  unitRecords.forEach(u=>{
    ...
    const constructionC=V?u.buildCost+modelC:0,permitAuthorityC=V?u.permitCost:0,contC=V?u.contCost:0;
    ...
    const vertC=V?(u.buildCost+u.permitCost+verticalDesignC+modelC+contC):0;
    const hardRiskC=phaseEligible[u.phase]*pw+(V?u.buildCost+modelC+contC:0),pidReduction=phaseReimb[u.phase]*pw;
  ...
  for(let m=0;m<=NM;m++){
    ...
    const otherEligible=R.dd[m]+R.infra[m]+R.cont[m]+R.vcont[m]+R.supervision[m]+R.pidprocess[m]+R.otherdirect[m]+R.build[m]+R.design[m]+R.permit[m]+R.model[m]+R.capex[m]+R.sellcash[m]+R.marketing[m];
  ...
  const finBase=(d.finland?land+closing+rollback+firb:0)+(d.findd?dd+soft+d.pidcost+otherDirect:0)+(d.fincapex?d.dacapex:0)+(d.finhoriz?infTot+horizCont+munic+impactTotal:0)+(d.finvert?actualBuild+actualPermit+modelCost+vertCont:0),ltcCommit=d.facmode&&d.facilityfixed>0?d.facilityfixed:d.ltc/100*finBase;
  ...
  const rows={
    ...
    land:by(R.landbasecogs),infra:by(infraNet),design:by(R.designcogs),authority:by(R.authoritycogs),construction:by(constructionPL),vertCont:by(R.contcogs),capitalisedFinance:by(capitalisedFinancePL),otherDirect:by(otherDirectPL),sell:by(R.sell),
  ...
  fys.forEach(y=>{
    ...
    const constructionAndCont=Math.max(1,infTot+horizCont),constructionCost=rows.infra[y]*infTot/constructionAndCont+rows.construction[y],contingencyCost=rows.infra[y]*horizCont/constructionAndCont+rows.vertCont[y];
  ...
  fys.forEach(y=>{
    ...
    const proxy=months.map(m=>{
      ...
      const dc=R.landbasecogs[m]+R.infracogs[m]+R.pidcogs[m]+R.designcogs[m]+R.authoritycogs[m]+R.contcogs[m]+constructionPL[m]+capitalisedFinancePL[m]+otherDirectPL[m]+R.sell[m]+R.marketing[m]+realEstateTaxPL[m]+R.gstcogs[m];
  ...
  for(let m=0;m<=NM;m++){
    ...
    R.cfinfra[m]=R.infra[m]+R.cont[m]+R.vcont[m];
    ...
    /* Statement presentation: the reference layout has no separate line for display cost,
       depreciable capex or builder-risk insurance, so Construction Cost carries them -- every
       cash item must map exactly once, and the mode is not a proxy for which rows are live.
       A vertical scheme can still carry horizontal infrastructure (infl is per lot in both
       modes) and can still earn an infrastructure reimbursement, so both are unconditional.
       Only costs inside the input-GST base are grossed up; insurance is not, and the
       reimbursement is a receipt, so both stay at face value. */
    R.cfrefconstruction[m]=(R.infra[m]+R.build[m]+R.model[m]+R.capex[m])*gstGrossFactor+R.insurance[m]-R.pid[m];
    ...
    R.cfrefcontingency[m]=(R.cont[m]+R.vcont[m])*gstGrossFactor;
  ...
  const modelChecks={
    ...
    noDevelopmentBeforeLand:[R.cinf,R.cont,R.vcont,R.munic,R.impact,R.build,R.design,R.permit,R.model].every(row=>noActivityBefore(row,close)),
    ...
    directCostClassification:Math.abs(S("dc")-(sum("landbasecogs")+sum("infracogs")+sum("pidcogs")+sum("designcogs")+sum("authoritycogs")+sum("constructioncogs")+sum("contcogs")+sum("insurancecaprel")+sum("insuranceexp")+sum("fincaprel")+sum("otherdirectcogs")+sum("sell")+sum("marketing")+sum("retaxcaprel")+sum("retaxexp")+sum("gstcogs")))<1,
...
export {
  ...
  // fyOf maps a month index to its financial year. Exported so the
  // statements use the model's OWN year boundaries rather than a second
  // implementation that could disagree about when FY starts.
  fyOf,
};
export default run;
