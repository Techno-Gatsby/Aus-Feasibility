/* Ports api/seed-au.sql (PostgreSQL) to T-SQL for Azure SQL Database.
   Mechanical and checked: dollar quoting -> N'' with doubled quotes, ::jsonb
   dropped, timestamptz + interval -> dateadd, ON CONFLICT DO NOTHING -> an
   IF NOT EXISTS guard so a second run is still a no-op. */
import { readFileSync, writeFileSync } from "node:fs";

const ROOT = "c:/Users/jay.kadam/OneDrive - Sobha LLC/Desktop/Australia code/";
const src = readFileSync(ROOT + "api/seed-au.sql", "utf8");

/* 1 - lift every $j$...$j$ literal out before anything else touches the text,
   so a stray quote inside an envelope cannot be confused with SQL syntax. */
const lits = [];
let body = src.replace(/\$j\$([\s\S]*?)\$j\$/g, (_, v) => `\u0000LIT${lits.push(v) - 1}\u0000`);

/* 2 - statement-level rewrites */
body = body
...
/* 3 - ON CONFLICT -> IF NOT EXISTS, keeping "safe to run twice" true */
body = body.replace(
...
/* 4 - put the literals back, as T-SQL. The envelopes run to ~15 kB each, so the
   JSON ones are cast explicitly to nvarchar(max) rather than left to the 4000
   character default a literal would otherwise take. */
const q = (s) => "N'" + s.replace(/'/g, "''") + "'";
body = body
  .replace(/\u0000JSON(\d+)\u0000/g, (_, i) => `cast(${q(lits[Number(i)])} as nvarchar(max))`)
  .replace(/\u0000LIT(\d+)\u0000/g, (_, i) => q(lits[Number(i)]));

const header = `/* Australian projects for Azure SQL Database - ported from seed-au.sql.
   ...
   GENERATED from seed-au.sql. Regenerate rather than hand-edit.
*/

`;

const out = header + body.replace(/^\/\*[\s\S]*?\*\/\n\n/, "");
writeFileSync(ROOT + "api/seed-au.sqlserver.sql", out);

/* 5 - checks that the port is complete */
const n = (re) => (out.match(re) || []).length;
const checks = [
  ["17 appraisal guards",       n(/if not exists \(select 1 from dbo\.appraisal where id/g) === 17],
  ["17 version guards",         n(/if not exists \(select 1 from dbo\.appraisal_version where/g) === 17],
  ["34 inserts",                n(/insert into dbo\.appraisal/g) === 34],
  ["no dollar quoting left",    !/\$j\$/.test(out)],
  ["no ::jsonb left",           !/::jsonb/.test(out)],
  ["no on conflict left",       !/on conflict/.test(out)],
  ["no interval left",          !/interval '/.test(out)],
  ["17 dateadd pairs",          n(/dateadd\(second,/g) === 51],
