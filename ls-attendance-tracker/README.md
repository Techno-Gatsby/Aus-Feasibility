...
`Attendance_Tracker.html` is one self-contained page for Latinem Securities attendance and client billing.
To use it, copy the file into `OneDrive - Sobha LLC\Desktop\LS_Documents` and open it in **Microsoft Edge** or **Chrome**.

## First run
1. Click **Choose data folder** (top right) and pick `LS_Documents`. The data is saved there as `tracker-data.json`, and a copy is kept each day in `tracker-backups\`. OneDrive syncs both files.
...
## Development
Edit the files in `src/`, then run `node build.mjs` to rebuild `Attendance_Tracker.html`.
